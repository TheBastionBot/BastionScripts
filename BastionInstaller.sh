...
then
  ...
  apt-get install -y wget curl checkinstall &>/dev/null
...
then
  ...
  yum -y install wget curl checkinstall &>/dev/null
...
then
  ...
  dnf -y install wget curl checkinstall &>/dev/null
...
(cd $INS_DIR && cd Bastion && npm install &>/dev/null) || (echo -e "${CYAN}[Bastion]: ${RED}[ERROR] Unable to download and install Bastion system dependencies.${NC} Check your internet connection." && exit 1)
